...
**Firebase Auth** gère l'authentification des utilisateurs. Ici configuré avec Google Sign-In : seul le compte Google autorisé peut accéder aux données. La session est mémorisée — la reconnexion est automatique après le premier login.
...
```bash
# 1. Modifier SRC/hud_todo_vX.html → incrémenter en vX+1 si modification significative
